...
`cube` supports several startup modes controlled by flags. The TUI is only launched in interactive mode; other modes produce plain stdout output.

| Flag | Behavior |
|------|----------|
| `cube` | Launch interactive TUI |
| `cube -p "prompt"` | Non-interactive: print response to stdout, no TUI |
| `cube --plan "task"` | Start in plan mode (read-only) |
| `cube -c` | Resume the most recent session |
| `cube -r` | Pick a session from a list |
| `cube -r <id>` | Resume a specific session directly |
| `cube -c --fork` | Fork the most recent session |
| `cube -r <id> --fork` | Fork a specific session |
| `cube --plugin-dir PATH` | Load plugins from a directory |
| `cube version` | Print version string |
| `cube help` | Print help |
...
```
TestVersionCommand                — cube version prints version string without provider
TestHelpCommand                   — cube help shows usage text
...
tmux send-keys -t t_cli 'cube' Enter
sleep 2
...
tmux send-keys -t t_cli 'cube -p "what is 1+1"' Enter
sleep 5
tmux capture-pane -t t_cli -p
# Expected: "2" on stdout; no TUI launched

# Test 3: Plan mode startup
tmux send-keys -t t_cli 'cube --plan "analyze this project"' Enter
sleep 2
tmux capture-pane -t t_cli -p
...
tmux send-keys -t t_cli 'cube -r' Enter
sleep 2
tmux capture-pane -t t_cli -p
...
tmux send-keys -t t_cli 'cube -c' Enter
sleep 2
tmux capture-pane -t t_cli -p
...
tmux send-keys -t t_cli 'cube version' Enter
sleep 1
tmux capture-pane -t t_cli -p
# Expected: version string printed to stdout

# Test 8: Help command
tmux send-keys -t t_cli 'cube help' Enter
sleep 1
tmux capture-pane -t t_cli -p
...
PEOF
tmux send-keys -t t_cli 'cube --plugin-dir /tmp/cli_plugin_test' Enter
sleep 2
...
tmux send-keys -t t_cli 'cube -c --fork' Enter
sleep 2
tmux capture-pane -t t_cli -p
...
tmux send-keys -t t_cli C-c
tmux send-keys -t t_cli "cube -r ${SESSION_ID} --fork" Enter
sleep 2
tmux capture-pane -t t_cli -p
