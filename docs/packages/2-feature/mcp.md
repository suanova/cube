...
MCP is how Cube talks to **external** tool providers (file system
...
Config lives at `<project>/.cube/mcp.json` (Cube) or
...
| **ConfigStore** — load / edit / save server definitions | free functions `PrepareServerEdit` / `ApplyServerEdit` | `cube mcp edit` CLI subcommand |
